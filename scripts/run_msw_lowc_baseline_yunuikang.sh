#!/usr/bin/env bash
# M-SWP-LOWC-BASELINE — TA + TA+O r2 low-concurrency comparison sweep (goguma6).
#   Fills baselines beside the MORI r2 low-C run: C in {2,4,8,10}.
#   TA   = --router tr, HiCache OFF (RATIO=0, EVICT=lru)
#   TA+O r2 = --router tr, --enable-hierarchical-cache --hicache-ratio 2, EVICT=lru
#   (r1/SMG skipped — low value at low C.)
# APPENDS to results_msw_lowc.jsonl (never truncates; MORI r2 cells preserved).
# Resume = run_tag skip. All other config identical to the MORI r2 low-C run.
set -uo pipefail
...
mkdir -p "$OUT"
# NEVER truncate — this run appends TA/TAO cells beside the existing MORI cells.
...
echo "==[M-SWP-LOWC-BASELINE] start $(date) pin=$PIN dur=${DUR}s (TA + TA+O r2, C in 2,4,8,10) =="
boot_backend lru 0 || exit 1
for C in 2 4 8 10; do run_cell "TA_r0_C$C"  TA  tr "" $C; done
boot_backend lru 2 || exit 1
for C in 2 4 8 10; do run_cell "TAO_r2_C$C" TAO tr "" $C; done
kill_proxy; kill_backend
echo "==[M-SWP-LOWC-BASELINE-DONE] $(date) results=$RES =="
