...
async def run_program(client, args, padder: Padder, program_id: str,
    ...
    async with sem:
        ...
        for rec in turns:
            ...
            tool_s = float(rec.get("tool_duration_s") or 0.0) * args.tool_scale
...
def main() -> None:
    ...
    ap.add_argument("--tool-scale", type=float, default=1.0,
                    help="expC: multiply every tool_duration_s by S (duty_cycle knob; "
                         "S<1 -> less tool -> higher d -> higher R). Cache untouched.")
