...
## 12. 저동시성 baseline 비교 (TA · TA+O r2 · MORI r2, C∈{2,4,8,10}) — 크로스오버  [측정, 2026-08-02]

원시: `results_msw_lowc.jsonl`(12셀 저C, **fail% 0**) + 기존 §1 고C(C20/50/80) 접합. out tok/s / TTFT p50(s), oversub=C/8.1.

| C | oversub | TA out/p50 | TA+O r2 out/p50 | MORI r2 out/p50 | MORI/TAO out | MORI/TAO p50 |
|---|---|---|---|---|---|---|
| 2 | 0.25× | 2.99 / 0.57 | 3.00 / 0.59 | 2.95 / 0.59 | 0.98 | 1.01 |
| 4 | 0.49× | 9.14 / 0.65 | 8.95 / 0.68 | 9.06 / 0.65 | 1.01 | 0.95 |
| 8 | 0.99× | 20.26 / 0.83 | 20.56 / 0.81 | 21.13 / 0.80 | 1.03 | 0.99 |
| 10 | 1.23× | 22.27 / 1.03 | 23.29 / 0.97 | 23.65 / 0.95 | 1.02 | 0.98 |
| 20 | 2.47× | 21.80 / 2.00 | 19.90 / 1.80 | 22.20 / 2.10 | **1.12** | 1.17 |
| 50 | 6.17× | 12.80 / 2.50 | 16.10 / 2.10 | 11.80 / 5.00 | **0.73** | 2.38 |
| 80 | 9.88× | 11.40 / 2.70 | 14.30 / 3.40 | 6.50 / 13.90 | **0.45** | 4.09 |

저C util%(warmup 20% 제외, 두 GPU 평균): C2 전 시스템 7 · C4 24–25 · C8 TA51/TAO46/MORI53 · C10 TA61/TAO56/MORI59 — **저C util도 세 시스템 동일(미포화)**.

**해석**:
- **(a) 저C 상승 = 공통(미포화).** C2~10에서 TA·TA+O·MORI의 out/TTFT/util이 **소수점까지 겹침**(예 C8 out 20.3/20.6/21.1). 저C throughput 상승은 부하 미포화 해소일 뿐 MORI 특성 아님.
- **(b) MORI vs TA+O 크로스오버 = oversub 2.5~6× 사이(C20~50), fit 경계(oversub 1) 아님.** throughput 비 0.98→1.03(C2~10 parity) → **C20 1.12(MORI 오히려 우위)** → C50 0.73 → C80 0.45. TTFT는 C10까지 parity → C20부터 MORI 소폭 열세(1.17×) → C50↑ 급격(2.38/4.09×). **즉 MORI는 fit을 넘어 oversub~2.5×까지 경쟁력 유지, oversub~6×(C50)에서 결정적으로 붕괴.**
- **(c) oversub<1 (C≤8): MORI ≈ TA.** throughput 0.98~1.03, TTFT parity → 저압박서 3-tier 오버헤드 무의미, 기계 정상 가동.
- **부가**: 고C서 **TA(offload 없음)가 TTFT 최저**(80: 2.7 vs TAO 3.4 vs MORI 13.9)이고 throughput도 MORI보다 높음(11.4 vs 6.5) → **MORI 3-tier 기구가 고압박에선 단순 pause/resume(TA)보다도 순손해**. offload 이득은 TA+O(16.1/14.3)에서만. util 저C 동일(미포화) ↔ 고C MORI 90% churn(§8) 대비.
- **§11 정정·정밀화**: §11의 "붕괴 oversub≥2.5×"는 MORI 절대치 기준. baseline 대비 **상대 크로스오버는 그보다 늦은 oversub~3–6×**(C20선 MORI가 오히려 앞섬). MORI 절대 TTFT 열화 시작(fit~oversub1.2)과 TA+O 대비 역전(oversub~3–6×)은 별개.

**소표본 신뢰도**: C=2는 전 시스템 completed 6 / turns 98 → 절대 throughput(~3)은 **미포화**(2워커·idle-heavy)이지 성능차 아님. 세 시스템 동값이라 상대비교는 유효. C≥4(25+ prog / 563+ turns) 표본 충분.

---

> 상태: 1런 패스 + 정량 진단(결함 1·2) + 수정설계 + **저동시성 곡선(§11) + baseline 크로스오버(§12)** 완료·정지. **GPU 유휴, 코드 무수정.** rootcause·regime-match 로그를 본 로그로 통합(중복 제거). 구현/재런치는 방향 결정 후 승인 시.
