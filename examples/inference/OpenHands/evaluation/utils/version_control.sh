checkout_eval_branch() {
    # Some environments package OpenHands without a git checkout (no .git/).
    # In that case, treat version switching as a no-op so evaluation can still run.
    if ! git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
        echo "Warning: Not a git work tree; skipping checkout_eval_branch."
        return 0
    fi
