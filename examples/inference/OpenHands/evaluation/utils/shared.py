...
def _safe_get_git_commit() -> str:
    """Return current git commit hash, or a placeholder when git metadata is unavailable.

    Some deployments remove `.git/` for packaging; evaluation should remain runnable.
    """
    # Allow explicit override (useful for reproducibility in packaged builds).
    override = os.environ.get('OPENHANDS_GIT_COMMIT')
    if override:
        return override
