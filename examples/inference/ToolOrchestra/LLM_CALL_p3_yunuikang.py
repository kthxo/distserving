...
def get_glm_client(timeout_s: float | None = None) -> OpenAI:
    # P3: free Z.ai GLM (paas/v4). key/base_url from env only (never logged).
    api_key = os.getenv("GLM_API_KEY")
    if not api_key:
        raise ValueError("GLM_API_KEY is required for GLM calls.")
    base_url = os.getenv("GLM_BASE_URL", "https://api.z.ai/api/paas/v4")
    if "openai/v1" in base_url:  # guard against the wrong /api/openai/v1 path
...
def get_llm_response(model,messages,temperature=1.0,return_raw_response=False,tools=None,show_messages=False,model_type=None,max_length=1024,model_config=None,model_config_idx=0,model_config_path=None,payload=None,**kwargs):
    ...
    # - Dedicated:  "HK123/Qwen2.5-Math-72B-Instruct-1d5fc83c"
    elif model_type == 'glm' or 'glm' in model.lower():
        # P3: free Z.ai GLM-4.5-Flash replacement for paid tools. thinking disabled
        # (else content is empty — all budget goes to reasoning_content). key/base_url from env.
        answer = ''
        updated_messages = []
        for m in messages:
            if 'tool_calls' in m:
                m['content'] = (m.get('content') or '') + str(m['tool_calls'])
                m.pop('tool_calls')
            updated_messages.append(m)
        retry_count = 0
        start_ts = time.time()
        while answer == '' and retry_count < max_retries:
            if call_timeout_s and (time.time() - start_ts) > call_timeout_s:
                raise LLMTimeoutError(f"GLM call exceeded {call_timeout_s}s (req_id={req_id}, model={model})")
            retry_count += 1
            try:
                glm_client = get_glm_client(timeout_s=(call_timeout_s or None))
                _kw = dict(model=model, messages=updated_messages, temperature=temperature,
                ...
                status = _get_status_code(error)
                if _is_context_length_error(error_str):
                    raise ContextLengthExceededError(f"GLM context length exceeded: {error}") from error
                if _is_auth_status(status):
                    raise MaxRetriesExceededError(f"GLM auth error (status={status}): {error}") from error
                if status == 400:
                    raise MaxRetriesExceededError(f"GLM invalid request (status={status}): {error}") from error
                if retry_count >= max_retries:
                    raise MaxRetriesExceededError(f"GLM max retries exceeded: {error}") from error
                sleep_s = 60 if _is_retryable_status(status) else min(5 * retry_count, 60)
                time.sleep(sleep_s)
        if answer == '' and not return_raw_response:
            raise MaxRetriesExceededError(f"GLM returned empty answer after {max_retries} attempts (req_id={req_id})")
        return answer
            ...
            while answer == '' and retry_count < max_retries:
                ...
                try:
                    ...
                    if tau2_stream_profile:
                        ...
                        # P3 FIX: our orchestrator is served via --served-model-name "orchestrator"
                        # (not a filesystem path), so os.path.exists() is False and tools were never
                        # sent -> 100% "invalid tool calls". Also accept the served name(s).
                        _orch_names = {"orchestrator"}
                        send_tools_to_vllm = bool(tools) and (
                            os.path.exists(str(model)) or str(model) in _orch_names
                        )
